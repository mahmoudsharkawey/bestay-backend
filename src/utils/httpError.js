...
export default function httpError(nextFunc, err, req, errorStatusCode) {
  ...
  if (!nextFunc || typeof nextFunc !== "function") {
    console.error(
      '🚨 CRITICAL ERROR: httpError called without valid "next" function!',
    );
    console.error(
      'This usually means the controller is missing the "next" parameter.',
    );
    console.error(
      "Controller signature should be: async (req, res, next) => {}",
    );
  ...
  const errorObject = {
    success: "false",
    statusCode: Number.isInteger(errorStatusCode)
      ? errorStatusCode
      : err.statusCode || 500,
